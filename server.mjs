...
const isAdmin = req => process.env.ADMIN_TOKEN ? req.headers['x-admin-token'] === process.env.ADMIN_TOKEN : req.headers['x-duty-role'] === 'admin';
const audit = (action, detail) => db.prepare('INSERT INTO audit_log(actor,action,detail) VALUES (?,?,?)').run('Администратор', action, detail);
...
async function api(req,res,url) {
  if (req.method==='GET' && url.pathname==='/api/bootstrap') { const s=snapshot(); s.permissions.canEdit=isAdmin(req); return json(res,200,s); }
  if (req.method==='POST' && url.pathname==='/api/duties') {
    if(!isAdmin(req)) return json(res,403,{error:'Изменять график может только администратор портала'});
    ...
    audit('Назначено дежурство', `${v.kind}: ${v.starts_on}–${v.ends_on}`); return json(res,201,snapshot());
  }
  if (req.method==='POST' && url.pathname==='/api/absences') {
    if(!isAdmin(req)) return json(res,403,{error:'Вносить отсутствие может только администратор портала'});
    ...
    audit('Добавлено отсутствие', `${hours} ч · ${v.occurred_on}`); return json(res,201,snapshot());
  }
