...
function render(){
  ...
  for(let day=1;day<=daysInMonth;day++){
    ...
    const events=[...list.map(duty=>`<div class="event ${duty.kind} ${state.permissions.canEdit?'admin-event':''}" title="${esc(duty.name)} · ${duty.kind==='office'?'офисное дежурство':`поддержка, ${duty.hours} ч`}">${ava(duty)}<span class="event-name">${esc(shortName(duty.name))}</span><b class="event-kind">${duty.kind==='office'?'офис':`${duty.hours} ч`}</b>${duty._count>1?`<span class="event-count" title="${duty._count} одинаковые записи">×${duty._count}</span>`:''}${state.permissions.canEdit?`<button class="delete" title="Удалить${duty._count>1?' одинаковые записи':''}" data-duty-ids="${duty._ids.join(',')}">×</button>`:''}</div>`),...absenceList.map(absence=>`<div class="event absence ${state.permissions.canEdit?'admin-event':''}" ${state.permissions.canEdit?`data-edit-absence="${absence.id}"`:''} title="${esc(absence.name)} · ${esc(absenceLabels[absence.absence_type]||'отсутствие')} · ${absence.hours} ч${absence.note?` · ${esc(absence.note)}`:''}">${ava(absence)}<span class="event-name">${esc(shortName(absence.name))}</span><b class="event-kind">${esc(absenceLabels[absence.absence_type]||'отсутствие')} · ${absence.hours} ч</b>${absence._count>1?`<span class="event-count" title="${absence._count} одинаковые записи">×${absence._count}</span>`:''}${state.permissions.canEdit?`<button class="delete" title="Удалить отсутствие${absence._count>1?' и его дубли':''}" data-absence-ids="${absence._ids.join(',')}">×</button>`:''}</div>`)].join('');
